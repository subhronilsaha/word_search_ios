...
enum Difficulty {
    ...
    var placementTypes: [PlacementType] {
        switch self {
        case .easy:
            return [.leftRight, .downUp].shuffled()
...
class Label { // Used as a reference type
...
class WordSearch {
    ...
    func makeGrid() {
        // Make empty 2D Array (grid) of labels
    ...
    private func tryPlacing(_ word: String, movement: (x: Int, y: Int)) -> Bool {
        ...
        for row in rows {
            for col in cols {
                let finalX = row + xLength
                let finalY = col + yLength
                
                if finalX >= 0 && finalX < gridSize && finalY >= 0 && finalY < gridSize {
                    if let returnValue = labels(fromX: row, y: col, word: word, movement: movement) {
...
//        return usedWords
    ...
    func render() -> Data {
        ...
        let gridLetterAtributes: [NSAttributedString.Key: Any] = [
            .font : gridLetterFont,
        ...
        return renderer.pdfData { (ctx) in
            for _ in (0 ..< numberOfPages) {
                ...
                for column in labels {
                    for label in column {
                        let size = String(label.letter).size(withAttributes: gridLetterAtributes)
                        ...
                        String(label.letter).draw(in: cellRect, withAttributes: gridLetterAtributes)
